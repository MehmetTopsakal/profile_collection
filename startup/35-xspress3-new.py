...
CommunityXspress3_1Channel = build_detector_class(
...
class CommunityXPDTomoXspress3Detector(CommunityXspressTrigger, CommunityXspress3_1Channel):
    ...
    hdf5 = Cpt(
        CommunityXspress3FileStore,
        "HDF1:",
        read_path_template="/nsls2/data/xpd/tomo/legacy/raw/canberra_data/%Y/%m/%d/",
        write_path_template="/nsls2/data/xpd/tomo/legacy/raw/canberra_data/%Y/%m/%d/",
        root="/nsls2/data/xpd/tomo/legacy",
    )
...
xs = CommunityXPDTomoXspress3Detector("XF:28ID2-ES{Xsp:1}:", name="xs")
...
for xs_channel in xs.iterate_channels():
    ...
    for xs_mcaroi in xs_channel.iterate_mcarois():
        mcaroi_read_attrs = [f"mcarois.mcaroi{m:02}.total_rbv" for m in range(1, 2)]
